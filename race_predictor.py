...
# Manual 2025 performance modifiers (1.0 = neutral)
driver_form_2025 = {
    'Max Verstappen': 0.96,
    'Charles Leclerc': 0.99,
    'George Russell': 0.98,
    'Lando Norris': 0.98,
    'Oscar Piastri': 0.97,
    'Kimi Andrea Antonelli': 1.00,
    'Lewis Hamilton': 1.00,
    'Carlos Sainz': 1.03,
    ...
    'Yuki Tsunoda': 1.02,
    'Oliver Bearman': 1.02,
    'Gabriel Bortoleto': 1.04,
    'Jack Doohan': 1.03,
    'Nico Hulkenberg': 1.02,
    'Liam Lawson': 1.03,
    'Pierre Gasly': 1.01,
    'Esteban Ocon': 1.02,
}
...
calendar_2025 = [
    ('Australian GP', 'Melbourne', False),
    ('Chinese GP', 'Shanghai', True),
    ('Japanese GP', 'Suzuka', False),
    ('Bahrain GP', 'Bahrain', False),
    ('Saudi Arabian GP', 'Jeddah', False),
    ('Miami GP', 'Miami', True),
    ('Emilia-Romagna GP', 'Imola', False),
    ('Monaco GP', 'Monaco', False),
    ('Spanish GP', 'Barcelona', False),
    ('Canadian GP', 'Montreal', False),
    ('Austrian GP', 'Spielberg', False),
    ('British GP', 'Silverstone', False),
    ('Belgian GP', 'Spa', True),
    ('Hungarian GP', 'Budapest', False),
    ('Dutch GP', 'Zandvoort', False),
    ('Italian GP', 'Monza', False),
    ('Azerbaijan GP', 'Baku', False),
    ('Singapore GP', 'Singapore', False),
    ('United States GP', 'Austin', True),
    ('Mexican GP', 'Mexico City', False),
    ('Brazilian GP', 'Sao Paulo', True),
    ('Las Vegas GP', 'Las Vegas', False),
    ('Qatar GP', 'Lusail', True),
    ('Abu Dhabi GP', 'Yas Marina', False)
]
...
def build_dnf_model(start_year=2021, end_year=2024):
    ...
    dnf_model = {}
    for key in starts:
        dnf_model[key] = dnf_counts[key] / starts[key] if starts[key] > 0 else 0
    return dnf_model

dnf_model = build_dnf_model()

print("Loading historical data (2021-2024)...")
df = build_dataset()
df['DriverForm'] = df['Driver'].map(driver_form_2025).fillna(1.00)
df['TeamForm'] = df['Team'].map(team_form_2025).fillna(1.00)
...
def evaluate_by_session_type(X_test, y_test, session_types, model):
...
evaluate_by_session_type(X_test, y_test, ['Q', 'R'], model)
...
def predict_2025_session(circuit, session_type):
    ...
    teams_2025 = {
        ...
        'Kimi Andrea Antonelli': 'Mercedes',
    ...
    for driver in drivers_2025:
        prediction_input.append({
            ...
            'Year': 2025,
            'Session': session_type,
            'DriverForm': driver_form_2025[driver],
            'TeamForm': team_form_2025[teams_2025[driver]]
        })
