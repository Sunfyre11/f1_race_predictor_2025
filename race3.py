...
form_influence = 10.0
teamform_scaling_factor = 0.5  # <<< You can tune this
...
# Define reduced feature list
feature_cols = ['Driver', 'Circuit', 'Year', 'Session', 'DriverForm']
target = 'Position'
X = df[feature_cols]
y = df[target]

categorical = ['Driver', 'Circuit', 'Session']
numerical = ['Year', 'DriverForm']
...
def predict_2025_session(circuit, session_type):
    ...
    df_pred['AdjustedPrediction'] = df_pred['PredictedPosition'] - teamform_scaling_factor * (df_pred['TeamForm'] - 10)
    df_pred['DNFChance'] = df_pred.apply(
        lambda row: dnf_model.get((row['Driver'], row['Circuit']), 0.02), axis=1
    )

    if session_type == 'R':
        df_pred['FinalPosition'] = df_pred['AdjustedPrediction'] + df_pred['DNFChance'] * 10
    ...
    for i, row in enumerate(df_pred.itertuples(), 1):
        if session_type == 'R':
            print(f"{i}. {row.Driver} ({row.Team}) - Predicted: {row.PredictedPosition:.2f} | Adjusted: {row.AdjustedPrediction:.2f} | DNF Chance: {row.DNFChance:.0%}")
        else:
